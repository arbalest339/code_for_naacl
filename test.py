...
def en_metrics(e1, e2, r, tag_seq):
    positive_true = [0, 0, 0]
    positive_false = [0, 0, 0]
    negative_false = [0, 0, 0]
...
def test():
    ...
    model.load_state_dict(torch.load("checkpoints/ore_zh_wodp.pkl"))
    ...
    positive_true = [0, 0, 0, 0]
    positive_false = [0, 0, 0, 0]
    negative_false = [0, 0, 0, 0]
    model.eval()
    for token, pos, ner, dp, head, matrix, e1, e2, r, mask in test_set.data:
        ...
        positive_true[3] += sum(pt)
        positive_false[3] += sum(pf)
        negative_false[3] += sum(nf)

        for i in range(3):
            positive_true[i] += pt[i]
            positive_false[i] += pf[i]
            negative_false[i] += nf[i]

    for i in range(4):
        precision = positive_true[i] / (positive_false[i] + positive_true[i])
        recall = positive_true[i] / (positive_true[i] + negative_false[i])
